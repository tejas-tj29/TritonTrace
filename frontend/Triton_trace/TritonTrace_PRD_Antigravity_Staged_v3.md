...
## 16.1 Overall Layout & Thematic Exception

**Layout:** TopHUD (with seamless role switcher) + edge-to-edge center map + left floating Fleet Radar + bottom-center floating 3-column Enterprise Dashboard.

**Thematic Exception:** Unlike the dark, tactical Investigator portal, the Commercial Operator floating panels must utilize a clean, white/light Enterprise design system (`bg-white`, `text-slate-900`, `border-slate-200`, `shadow-xl`). This provides a sharp visual contrast distinguishing the corporate risk interface from the forensic intelligence interface.

## 16.2 Commercial Fleet Radar (Left Panel)

A vertically docked floating panel on the left side of the screen.

**Contents:**
- Active vessel count badge.
- List of mock fleet assets (e.g., MV HELIOS LEADER, MT AEGEAN GLORY, MV NORDIC TRADER).
- Real-time telemetry per vessel: Speed, Heading, and Range to Slick (NM).
- Proximity warning badges (e.g., "ELEVATED WATCH").

**Interaction:** 
Clicking a vessel card updates the `selectedVesselId` in context, triggers a smooth map fly-to animation centering on the vessel, and mounts the Enterprise Dashboard.

## 16.3 Enterprise Dashboard (Bottom Center)

A wide, horizontally docked panel at the bottom center of the map. It is divided into three equal columns separated by borders. It only renders when a vessel is selected from the Fleet Radar.

**Column 1: Alibi Generator (Non-Involvement Audit)**
- **Purpose:** Prove the selected vessel did not cause the active slick.
- **UI:** `ShieldCheck` icon (blue accent).
- **Content:** Verifies vessel spatial clearance at T-0 (time of release).
- **Action:** `EXPORT AUDIT (JSON)` button downloads a mock JSON receipt proving the vessel's alibi to port authorities.

**Column 2: P&I Risk Assessor (Liability Exposure)**
- **Purpose:** Calculate financial exposure if the vessel enters the slick or a penalized geofence.
- **UI:** `AlertTriangle` icon (rose/red accent).
- **Content:** Displays the active geofence multiplier and dynamic liability math (e.g., "$15.4M EST. EXPOSURE" in large bold text). Base penalty × zone multiplier + area penalty.

**Column 3: Dynamic Rerouting (Spill Avoidance)**
- **Purpose:** Automated course correction.
- **UI:** `Route` icon (emerald accent).
- **Content:** Compute a 15nm safe-transit fairway avoiding the active slick and critical strike zones.
- **Action:** `EXECUTE SAFE DIVERSION` button. Clicking this toggles the diversion corridor visibility on the map canvas.

## 16.4 Map Integration & Geofence Routing

When the Commercial Portal is active, the Map Engine must dynamically render:
- **Fleet Assets:** Custom maritime cyan/blue ship icons, rotated dynamically via CSS to match their live heading.
- **Projected Trajectory:** A dashed amber/orange line projecting the vessel's current dangerous course toward the hazard zone.
- **Diversion Fairway:** When "Execute Safe Diversion" is triggered, render an alternate waypoint corridor as a dashed emerald green line (`#10b981`) bypassing the slick.
- **Regional Alert Geofences:** Integration of `regional_alert_geofences.json` rendering translucent Watch Zones (Amber, 50km) and Critical Strike Zones (Red, 15km) with interactive liability tooltips.   update the section 16
